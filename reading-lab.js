...
function readingMarkDefinition(mark) { const word = readingMarkWord(mark); return String(mark?.definition || word?.d || "").trim(); }
function readingMarkMeaning(mark) { return String(readingMarkDefinition(mark) || mark?.note || "").trim(); }
...
function readingMarkSummary(mark) {
  return readingMarkMeaning(mark) || "Context saved · look it up when you are ready";
}
...
async function lookUpReadingMark(markId) {
  ...
  if (readingLookupRequest?.mark === mark) return;
  cancelReadingLookup();
  readingFlow = { kind: "lookup", markId, loading: false };
  if (readingMarkDefinition(mark)) return readingLookupView(markId);
  if (query.replace(/\s+/g, " ").length > 280) return readingLookupView(markId, "Select a word, phrase, or short sentence (up to 280 characters).");
  if (window.location.protocol === "file:") return readingLookupView(markId, "Open Lexora through its launcher or npm start to connect explanations.");
  readingFlow.loading = true; readingLookupView(markId);
  const controller = new AbortController(), timeout = window.setTimeout(() => controller.abort(), 15_000);
  const request = { controller, mark };
  readingLookupRequest = request;
  const isCurrent = () => readingLookupRequest === request && readingDraft() === draft && readingMarks(draft).includes(mark) && readingFlow?.kind === "lookup" && readingFlow.markId === markId;
  try {
    const response = await fetch("/api/reading-lookup", { method: "POST", headers: { "Content-Type": "application/json" }, signal: controller.signal, body: JSON.stringify({ text: query, context: String(mark.context || query).slice(0, 1500) }) });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      const fallback = [404, 405, 501].includes(response.status) ? "This server does not run the explanation API. Locally, restart Lexora with npm start. For an online site, deploy its API." : response.status === 504 ? "The explanation service timed out. Please try again." : "The lookup service is unavailable.";
      throw new Error(payload?.error || fallback);
    }
    const definition = typeof payload.definitionEn === "string" ? payload.definitionEn.trim() : "";
    if (!definition) throw new Error("The lookup service returned an incomplete explanation. Please try again.");
    if (!isCurrent()) return;
    Object.assign(mark, { definition, partOfSpeech: String(payload.partOfSpeech || "").trim(), example: String(payload.studyCue || "").trim(), lookupAt: Date.now() });
    save(); readingFlow = { kind: "lookup", markId, loading: false }; readingLookupView(markId);
  } catch (error) {
    if (!isCurrent()) return;
    readingFlow = { kind: "lookup", markId, loading: false };
    const message = error?.name === "AbortError" ? "The lookup took longer than 15 seconds. Please try again." : error instanceof TypeError ? "Could not connect to explanations. Check your connection and that the Lexora server is running." : String(error?.message || "Check your connection and try again.");
    readingLookupView(markId, message);
  } finally { window.clearTimeout(timeout); if (readingLookupRequest === request) readingLookupRequest = null; }
}
function readingNoteModal(markId) {
  const draft = readingDraft(), mark = readingMarks(draft).find((item) => item.id === markId); if (!mark) return readingPassageView();
  const word = readingMarkWord(mark), definition = readingMarkDefinition(mark);
  root.innerHTML = `<div class="overlay" role="dialog" aria-modal="true"><section class="modal reading-note-modal"><button class="icon-button modal-close" data-action="reading-return-passage" aria-label="Close">×</button><div class="modal-intro"><p class="eyebrow">MARKED FROM READING</p><h2>${esc(readingMarkLabel(mark))}</h2><p class="reading-context">“${esc(mark.context || "Context was not captured.")}”</p>${!word && definition ? `<div class="reading-saved-lookup"><span>IN SIMPLE ENGLISH</span><b>${esc(definition)}</b></div>` : ""}<label class="field-label" for="reading-mark-note">${word ? "MEANING FROM YOUR WORD BANK" : definition ? "YOUR PERSONAL NOTE (OPTIONAL)" : "YOUR NOTE / EXPLANATION"}</label><textarea id="reading-mark-note" class="reading-mark-note-input" maxlength="500" placeholder="Write a short meaning or reminder…" ${word ? "readonly" : ""}>${esc(word ? definition : mark.note || "")}</textarea><div class="modal-footer"><span class="subtle-note">${word ? "This word already has a saved definition." : definition ? "The explanation is already saved to your review card." : "A note makes this selection into a clearer flashcard and quiz question."}</span><button class="modal-cta teal" data-action="reading-save-note" data-id="${mark.id}">${word ? "Back to passage" : "Save note"}</button></div></div></section></div>`;
}
...
function readingReviewView() {
  ...
  const total = readingFlow.queue.length, back = readingMarkMeaning(mark), context = word?.e || mark.context || "", label = readingMarkDefinition(mark) ? "IN SIMPLE ENGLISH" : "YOUR NOTE", definition = !word ? readingMarkDefinition(mark) : "";
...
function startReadingQuiz() {
  ...
  const items = shuffled(readingQuizItems(draft)).slice(0, 5); if (!items.length) return notice("Add a note or explanation to at least one marked phrase before starting a quiz.");
...
window.LexoraReadingLab = { clearSession: () => { cancelReadingLookup(); if (document.fullscreenElement) document.exitFullscreen().catch(() => {}); readingFlow = null; } };
