...
function makeQuestion(w) { const imported = randomItem(taskVariants(w)); const fallback = imported ? { prompt: imported.prompt, question: imported.question, options: imported.options, correct: imported.correct } : { prompt: "Which definition best matches this word?", question: "Choose the most precise definition.", options: [w.d, ...app.words.filter((x) => x.id !== w.id).slice(0,3).map((x) => x.d)], correct: 0 }; const options = fallback.options.map((t,n)=>({t,right:n===fallback.correct})).sort(()=>Math.random()-.5); return { w, prompt: fallback.prompt, question: fallback.question, options, correct: options.findIndex((o)=>o.right) }; }
function beginTest() { let pool = collectionWords(test.collection); if (document.querySelector("#hard-only").checked) pool = pool.filter(needsPractice); if (!pool.length) return notice("No words in this set need practice. Scan cards or choose another collection."); const count = +document.querySelector("#test-count").value, ordered = [...pool].sort(()=>Math.random()-.5); test = { ...test, qs: Array.from({length:count},(_,i)=>makeQuestion(ordered[i%ordered.length])), i:0, selected:null, answers:[], opening:true, animate:false }; testView(); }
function testView() { const opening = Boolean(test.opening), animate = Boolean(test.animate); test.opening = false; test.animate = false; const q=test.qs[test.i], p=Math.round(test.i/test.qs.length*100); root.innerHTML=`<div class="overlay ${opening ? "" : "session-overlay"}" role="dialog" aria-modal="true"><section class="modal session-modal ${opening ? "session-opening" : ""}"><div class="session-top"><span class="session-label">VOCAB TEST · QUESTION ${test.i+1} OF ${test.qs.length}</span><button class="icon-button session-close" data-action="close">×</button></div><div class="session-progress"><i style="width:${p}%;background:var(--purple)"></i></div><div class="session-body ${animate ? "task-enter" : ""}"><span class="step-tag" style="color:#d2b7ff;background:rgba(147,97,242,.12)">ANSWER NOW · REVIEW LATER</span><h2>Choose the best answer.</h2><p class="question-kicker">${esc(q.prompt)}</p><p class="question-kicker"><b>${esc(q.question || "Which answer is best supported by the text?")}</b></p><div class="answer-list">${q.options.map((o,i)=>`<button class="answer-option ${test.selected===i?"selected":""}" data-action="test-answer" data-id="${i}"><span class="option-letter">${String.fromCharCode(65+i)}</span>${esc(o.t)}</button>`).join("")}</div><div class="session-actions"><button class="modal-cta purple" data-action="next-test" ${test.selected===null?"disabled":""}>${test.i+1===test.qs.length?"Finish test →":"Next question →"}</button></div></div></section></div>`; }
